...
public:
  ...
  EncodeResult EncodeEx(const std::string &text, bool add_special_tokens) final {
    TokenizerEncodeExResult result;
    tokenizers_encode_ex(handle_, text.data(), text.length(), static_cast<int>(add_special_tokens),
                         &result);
    std::vector<int32_t> TokenIds(result.token_ids, result.token_ids + result.token_ids_len);
    std::vector<int32_t> TokenTypeIds(result.token_type_ids, result.token_type_ids + result.token_type_ids_len);
    std::vector<int32_t> Masks(result.masks, result.masks + result.masks_len);
    tokenizers_free_encode_ex_results(&result, 1);
    return {std::move(TokenIds), std::move(TokenTypeIds), std::move(Masks)};
  }
